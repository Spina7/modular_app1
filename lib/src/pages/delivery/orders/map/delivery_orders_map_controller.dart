...
import 'package:location/location.dart'
    as location; //Paquete complementario para establecer nuestra localizacion
...
class DeliveryOrdersMapController extends GetxController {
  ...
  CameraPosition initialPosition = CameraPosition(target: LatLng(20.7123519, -103.4113297), zoom: 14);
  ...
  void connectAndListen() {
    //METODO QUE NOS PERMITE CONECTARNOS A SOCKET IO
    socket.connect();
  ...
  void updateToDelivered() async {
    ...
    if (distanceBetween <= 200) {
      ...
      Get.snackbar('Operacion no Permitida',
          'Debes de estar mas cerca a la posicion del pedido');
    }
  }
  ...
  void updateLocation() async {
    ...
    try {
      ...
      position = await Geolocator
          .getLastKnownPosition(); //Podemos tener la latitud y longitud(ACTUAL)
      saveLocation();
      animateCameraPosition(position?.latitude ?? 20.7123519,
      ...
      positionSubscribe = Geolocator.getPositionStream(locationSettings: locationSettings).listen((Position pos) {
